...
> **Real-use lens — the top adoption lever is oracle synthesis** (surfaced from running loopkit as a
> remediation harness, so it sits outside the prior-art list below; full ranking in the
> `project_loopkit_next_features` roadmap). The biggest day-to-day friction is hand-authoring a
> fail-first held-out oracle per task. The fix: an **independent proposer** drafts the held-out check
> from the goal, and the harness **verifies it fails on the current tree before trusting it** — turning
> "author a test" into "review a proposed test," preserving independence (the proposer is not the loop
> under test). The manual now teaches this as the discipline's front-door friction (`loops/09`); build
> it as a `synth-gate`-style helper on the `--validate` preflight seam.

1. **A persistent agent scratchpad — the interesting tension with fresh-context.** [Manus](https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus) + Anthropic both land on **structured note-taking** (a `NOTES.md`/todo the agent writes and re-reads across resets) and *"keep the wrong stuff in"* (failed attempts are in-loop signal). loopkit's fresh-context is a strength (context-rot mitigation) but discards *intra-run* learning except via the single last-gate-feedback string — the agent rediscovers dead-ends each tick. A durable, agent-authored note channel rendered like an anchor would keep the fresh-context purity while adding memory. The manual teaches this as foundational — the *rediscovery* tax and the bounded note channel that repays it (`loops/05`). *Design before building — it touches the fresh-context philosophy.*
